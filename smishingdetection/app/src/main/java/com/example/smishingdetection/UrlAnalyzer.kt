...
class UrlAnalyzer(private val baseUrl: String) {
    ...
    suspend fun analyzeUrl(url: String?): Pair<ScanStatus, String> =
            ...
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    if (jsonResponse.has("uuid")) {
                        ...
                        return@withContext Pair(ScanStatus.SUCCESS,"$scanResult")
                    }
                } else {
                    ...
                    when (responseCode) {
                        HttpURLConnection.HTTP_BAD_REQUEST -> {
                            ...
                            return@withContext Pair(ScanStatus.FAILURE,"Unable to scan URL. urlscan.io returned $errorMessage")
                        }

                        HttpURLConnection.HTTP_INTERNAL_ERROR -> {
                            return@withContext Pair(ScanStatus.FAILURE, "$response")
                        }
                    }

                    return@withContext Pair(ScanStatus.FAILURE, "Unable to scan URL. API returned $response")
                }
            }
            catch (e: SocketTimeoutException) {
                Log.e("UrlAnalyzer", "Exception: ${e.localizedMessage}")
                return@withContext Pair(ScanStatus.FAILURE, "Scan timed out. URL verdict could not be retrieved.")
            }
            catch (e: ConnectException) {
                Log.e("UrlAnalyzer", "Exception: ${e.printStackTrace()}.")
                return@withContext Pair(ScanStatus.FAILURE, "Cannot connect to url sandbox.")

            }
            catch (e: Exception) {
                Log.e("UrlAnalyzer", "Exception: ${e.javaClass.simpleName} - ${e.message}\nCause: ${e.cause}\nTrace: ${e.printStackTrace()}")
            }
            return@withContext Pair(ScanStatus.FAILURE, "Smishing Detector failed to get scan result for $url")
        }
}